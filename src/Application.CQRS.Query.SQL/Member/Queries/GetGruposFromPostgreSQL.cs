        ...
        string result = $@"
            ...
            Navigator AS (
                SELECT
                    ...
                    (total_records / {PageSize}) + (CASE WHEN total_records % {PageSize} > 0 THEN 1 ELSE 0 END) AS PageCount FROM RecordCount
            ...
            /*Selecionando os registros da página especificada usando os parâmetros da CTE Navigator*/
            WITH RecordCount AS (
                SELECT COUNT(*) AS total_records FROM #Grupos#
            ),
            Navigator AS (
            ...
            {select}
              ORDER BY #Name#
              LIMIT (SELECT PageSize FROM Navigator)
              OFFSET ((SELECT PageNumber FROM Navigator) - 1) * (SELECT PageSize FROM Navigator);
        ";        
